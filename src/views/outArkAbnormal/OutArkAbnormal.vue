...
<template>
  <div>
    <OutArkAbnormalSearch ref="OutArkAbnormalSearch"></OutArkAbnormalSearch>
    <OutArkAbnormalTable ref="OutArkAbnormalTable" :tableData="tableData" :tableLoading="tableLoading">
    </OutArkAbnormalTable>
...
import OutArkAbnormalSearch from "@/components/warehouse/outArkAbnormal/OutArkAbnormalSearch.vue";
import OutArkAbnormalTable from "@/components/warehouse/outArkAbnormal/OutArkAbnormalTable.vue";
import { backTableTop } from "@/utils/util";
export default {
  name: "outArkAbnormal",
  components: { OutArkAbnormalSearch, OutArkAbnormalTable },
  setup(prop, ctx) {
    const data = reactive({
      tableData: [],
      tableLoading: false,
    ...
    onActivated(() => {
      // getTableData();
      setTimeout(() => {
        backTableTop("#OutArkAbnormalTable", data.scroll);
      }, 100);
      window.addEventListener("scroll", handleScroll, true);
    });
    onDeactivated(() => {
    ...
    const handleScroll = () => {
      data.scroll = document.querySelector(
        `#OutArkAbnormalTable .el-table .el-table__body-wrapper`
      ).scrollTop;
    };
    ...
    const getTableData = (msg) => {
      ...
      if (data.searchMsg) {
        ...
        // info.cabinetAbnormalType = data.searchMsg.cabinetAbnormalType ? data.searchMsg.cabinetAbnormalType : vue.$route.query.type ? vue.$route.query.type : '';
        // info.cabinetNum = data.searchMsg.cabinetNum ? data.searchMsg.cabinetNum : vue.$route.query.cabinetNum ? vue.$route.query.cabinetNum : '';
      }
      api.warehouse
        .getCabinetAbnormal(info)
        .then((res) => {
          if (res.code == 200) {
