<!-- 调拨异常 -->
<template>
  <div>
    <AllocatingAbnormalSearch ref="AllocatingAbnormalSearch" :searchMsg="searchMsg">
    </AllocatingAbnormalSearch>
    <AllocatingAbnormalTable ref="AbnormalWarehousingTable" :searchMsg="searchMsg" :tableData="tableData"
      :tableLoading="tableLoading"></AllocatingAbnormalTable>
    <Pagination :total="myPages.total" :limit="myPages.pageSize" :page="myPages.pageNum"
      @update:limit="myPages.pageSize = $event" @update:page="myPages.pageNu = $event" :hidden="myPages.total <= 0"
      @handleCurrentChange="handleCurrentChange" @handleSizeChange="handleSizeChange"></Pagination>
  </div>
</template>

<script>
import { reactive, toRefs, ref, onBeforeMount, onMounted, getCurrentInstance, provide, nextTick, onActivated, onDeactivated, onBeforeUnmount } from "vue";
import AllocatingAbnormalSearch from "@/components/warehouse/allocatingAbnormal/AllocatingAbnormalSearch.vue";
import AllocatingAbnormalTable from "@/components/warehouse/allocatingAbnormal/AllocatingAbnormalTable.vue";
import authorButtons from "@/compositionApi/authorButtons";
import { backTableTop } from "@/utils/util";
export default {
  name: "allocatingAbnormal",
  components: { AllocatingAbnormalSearch, AllocatingAbnormalTable },

  setup(prop, ctx) {
    const { BUTTONS } = authorButtons();
    const buttonAuthor = BUTTONS.value;
    const data = reactive({
      tableData: [],//列表数据
      tableLoading: false, // 表格loading
      myPages: {
        pageSize: 10,
        pageNum: 1,
        pages: 1,
        total: 0,
      },
      scroll: 0,
      tableInfo: {
        // 要传给后台的数据
        current: 1, // 当前页为 1
        size: 10, // 每页10条
      },
      searchMsg: {
        current: 1, // 当前页为 1
        size: 10, // 每页10条
      },
    });
    const { ctx: vueDev, proxy: vue } = getCurrentInstance();
    const api = vue.$http;
    onBeforeMount(() => { });
    onMounted(() => {
      getTableList(data.tableInfo);
      window.addEventListener("scroll", handleScroll, true);
    });
    onActivated(() => {
      getTableList(data.tableInfo);
      setTimeout(() => {
        backTableTop("#AllocatingAbnormalTable", data.scroll);
      }, 100);
      window.addEventListener("scroll", handleScroll, true);
    });
    onDeactivated(() => {
      window.removeEventListener("scroll", handleScroll, true);
    });
    onBeforeUnmount(() => {
      window.removeEventListener("scroll", handleScroll, true);
    });
    const handleScroll = () => {
      data.scroll = document.querySelector(`#AllocatingAbnormalTable .vxe-table--body-wrapper .vxe-table--body`).scrollTop;
    };
    // 获取列表数据
    const getTableList = msg => {
      data.searchMsg = msg || data.searchMsg;
      let info = {};
      if (data.searchMsg) {
        info = data.searchMsg;
      }
      data.tableLoading = true;
      console.log(info, "info");
      api.requisitionManagement
        .getAbnormalWarehousing(info)
        .then(res => {
          if (res.code == 200) {
            data.tableData = res.data.records;
            data.tableLoading = false;
            data.tableInfo.current = res.data.current;
            data.tableInfo.size = res.data.size;
            data.myPages.pageSize = res.data.size;
            data.myPages.pageNum = res.data.current;
            data.myPages.pages = res.data.pages;
            data.myPages.total = res.data.total;
            if (msg && msg.type == 0) {
              data.scroll = 0;
            }
            nextTick(() => {
              backTableTop("#AllocatingAbnormalTable", data.scroll);
            });
          } else {
            vue.$message.warning(res.msg);
            data.tableLoading = false;
          }
        })
        .catch(err => {
          data.tableLoading = false;
        });
    };

    provide("getTableList", getTableList);
    const handleSizeChange = val => {
      data.tableInfo.size = val;
      let info = data.searchMsg;
      info.size = data.tableInfo.size;
      info.current = data.tableInfo.current;
      data.searchMsg = info;
      data.scroll = 0;
      getTableList(data.searchMsg);
    };
    const handleCurrentChange = val => {
      data.tableInfo.current = val;
      let info = data.searchMsg;
      info.size = data.tableInfo.size;
      info.current = data.tableInfo.current;
      data.searchMsg = info;
      data.scroll = 0;
      getTableList(data.searchMsg);
    };
    const refData = toRefs(data);

    return {
      ...refData,
      getTableList,
      buttonAuthor,
      handleSizeChange,
      handleCurrentChange,
    };
  },
};
</script>
<style scoped lang="scss">

</style>
