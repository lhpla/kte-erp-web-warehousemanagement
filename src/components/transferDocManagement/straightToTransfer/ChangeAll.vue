...
<template>
  <div id="ChangeAll">
    <el-dialog v-model="UpdateDialogVisible" title="调拨入库-批量修改尺寸" width="60%" @close="handleClose" destroy-on-close
      ...
      <el-form ref="ruleFormRef" :model="ruleForm" label-width="80px" :inline="true" @submit.native.prevent>
        <el-form-item prop="sku" label="SKU: " @dblclick="openInput(ruleForm.sku, 'sku')">
          <el-input v-model="ruleForm.sku" clearable placeholder="双击批量查询" @keyup.enter="search" maxlength="500">
          </el-input>
        </el-form-item>
  ...
  setup(prop, ctx) {
    ...
    const getList = () => {
      ...
      api.requisitionManagement
        .getWholeSizeDetail(info)
        .then((res) => {
          if (res.code == 200) {
            data.tableData = res.data;
            data.tableData.map((item, index) => {
              item.index = index;
              item.singleCartonCount = item.singleCartonCount == -1 ? 0 : item.singleCartonCount;
            });
          }
    ...
    const onSubmit = () => {
      ...
      data.tableData.forEach((row) => {
        if (vue.$refs[`size_${row.index}`]) {
          validateMethods.push(vue.$refs[`size_${row.index}`].validate());
        }
      });
