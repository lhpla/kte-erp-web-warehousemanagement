      ...
      <el-form ref="ruleFormRef" :model="ruleForm" label-width="80px" :rules="rules" :inline="true">
        <el-form-item prop="cabinetNum" label="柜号：">
          <el-select v-model="ruleForm.cabinetNum" clearable placeholder="请选择">
            <el-option v-for="item in cabinetNumList" :key="item" :label="item" :value="item"> </el-option>
          </el-select>
        </el-form-item>
...
export default {
  ...
  setup(prop, ctx) {
    ...
    const openDialog = (list, resInfo, transferCode) => {
      console.log(list, resInfo, transferCode);
        ...
        if (valid) {
          if (data.selectList.length !== 0) {
            ...
            api.requisitionManagement
              .batchUpdateAllCabinetNumber({ transferCode: data.transferCode, CabinetNum: data.ruleForm.cabinetNum })
              .then((res) => {
                if (res.code == 200) {
                  updateMsg(data.transferCode);
                  vue.$message.success(res.msg);
