<!-- 批量修改柜号 -->
<template>
  <div id="BatchUpdateCabinetNum">
    <el-dialog v-model="UpdateDialogVisible" title="调拨入库-修改柜号" width="30%" @close="handleClose" destroy-on-close
...
export default {
  ...
  setup(prop, ctx) {
    ...
    //打开弹窗
    const openDialog = (resInfo, transferCode) => {
    ...
    const onSubmit = () => {
      ruleFormRef.value.validate((valid) => {
        if (valid) {
          ...
          api.requisitionManagement.batchUpdateAllCabinetNumber(info).then((res) => {
            if (res.code == 200) {
              getMsg()
