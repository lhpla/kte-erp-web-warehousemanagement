<!-- 发货计划下载列表 DownListTable-->
<template>
  <div id="PurchaseDownListTable">
    <el-dialog title="下载队列" v-model="downVisible" width="80%" :close-on-click-modal="false" @close="handleClose" :close-on-press-escape="false">
      <el-table v-loading="tableLoading" :data="tableData" border style="width: 100%"
        :header-cell-style="{ background: '#fafafa', color: '#2d2f30', fontWeight: 'bold', fontSize: '12px' }">
        <el-table-column label="文件名称" prop="fileName"></el-table-column>
        <el-table-column label="文件来源" prop="fileSource"></el-table-column>
        <el-table-column label="创建人" prop="createUser"></el-table-column>
        <el-table-column label="状态" prop="state">
          <template #default="scope">
            <div>
              {{ scope.row.state == 0 ? "正在处理" : scope.row.state == 1 ? "已完成" : "失败" }}
            </div>
          </template>
        </el-table-column>
        <el-table-column label="操作时间">
          <template #default="scope">
            <div>
              ...
              <div>完成时间：{{ scope.row.completionTime ? scope.row.completionTime : "-" }}</div>
            </div>
          </template>
        </el-table-column>
        <el-table-column label="操作" prop="cardBank">
          <template #default="scope">
            <el-button  type="primary" text icon="el-icon-download" v-if="scope.row.state == 1"
      ...
      <Pagination :total="myPages.total" :limit="myPages.pageSize" :page="myPages.pageNum"
        ...
        @handleCurrentChange="handleCurrentChange" @handleSizeChange="handleSizeChange" class="PurchaseDownListTable">
      </Pagination>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  setup(prop, ctx) {
    ...
    const getDownloadQueue = msg => {
      ...
      api.warehouse.getDownloadQueue(data.searchMsg).then(res => {
        if (res.code == 200) {
