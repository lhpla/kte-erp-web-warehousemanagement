...
<template>
  <div id="PurchaseDownListTable">
    <el-dialog title="下载队列" v-model="downVisible" width="80%" :close-on-click-modal="false" :before-close="handleClose" :close-on-press-escape="false">
      <el-table
      ...
      >
        ...
        <el-table-column label="操作" prop="cardBank">
          <template #default="scope">
            <el-button size="small" type="text" icon="el-icon-download" v-if="scope.row.state == 1" @click="handelDownFile(scope.row.fileLink)">下载</el-button>
            <span v-else>-</span>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  setup(prop, ctx) {
    ...
    const getDownloadQueue = msg => {
      ...
      api.orderList.getOffDownloadQueue(data.searchMsg).then(res => {
        if (res.code == 200) {
